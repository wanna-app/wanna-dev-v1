...
- **Mobile mod UI does NOT send the user-facing email yet.** `mod_resolve_report` only writes the resolution + the new fields and deactivates the user for bans; the actual email render + Resend send lives in the `moderate-user` edge function which is service-role only. Wiring the in-app flow to fire that email (e.g. via a SECURITY DEFINER plpgsql wrapper using pg_net + the vault-stored service-role key) is a follow-up. The new columns are already populated on the row, so when the email path is wired, the values are ready.
