...
- **Moderator-overridable report fields (2026-05-06):** Migration `00043_reports_moderator_fields.sql` adds three nullable columns to `reports` — `removed_content_type` (activity/photo/message), `ban_duration` (custom human-readable duration, e.g. "24 hours"), `ban_reason` (short explanation shown to user, e.g. "Harassment in chat after prior warning"). `moderate-user` accepts these as optional request fields, persists them on the report row alongside the resolution, and uses them as overrides in the email render with sensible fallbacks (action-canonical duration label, reporter's reason, reported_content_type). Moderator UI (ReportsQueueScreen) doesn't yet expose form fields for these — call sites still pass only `action` + `reason` and get the same behavior as before. **TODO:** add inputs to the moderator action dialog so these can be set from the dashboard.
