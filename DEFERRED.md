...
- **What's broken:** the `ResolveReportModal` (in-app Mod tab) calls the `mod_resolve_report` Postgres RPC. That RPC writes the report row, persists the new `removed_content_type` / `ban_duration` / `ban_reason` fields, deactivates the user for bans, **revokes their auth.sessions**, and (for permanent bans) **sets `auth.users.banned_until` to a far-future date** so they can't sign back in (migration 00045) — but it **does not send the user-facing email**. The Resend send lives in the `moderate-user` edge function, which is service-role only and can't be called from a regular user's mobile app.
- **What this means right now:** moderators using the in-app modal silently skip the warning / content-removed / suspension / permanent-ban email to the user. The data is captured correctly, the active sessions are killed, the permanent-ban flow blocks re-signin via both `banned_emails` (re-signup) and `auth.users.banned_until` (re-signin), but the user never hears about any of it via email.
...
- **Then:** once wired, **revise [the user-facing moderation guide](your standalone doc)** so it can describe the in-app modal as a single one-stop flow ("pick action → fill fields → tap Resolve → email goes out + sessions die + signin blocked"). The current draft of the guide that describes manual Supabase Table-Editor edits is obsolete the moment that wiring lands. Owner-action item to update the guide is parked here so it doesn't get lost. Note: the manual "Authentication → Users → Ban user" Dashboard step in the existing draft is no longer needed for permanent bans — `mod_resolve_report` handles `auth.users.banned_until` directly as of migration 00045.

### Temp-ban duration on the in-app flow is not actually enforced yet
- **What's broken:** the modal collects a duration (24h / 7d / 30d preset or custom string), `mod_resolve_report` persists it on the report row, and the user's session is killed — but **`profiles.banned_until` is not set**, so the `auto-unban` cron never sees a temp-ban to expire and the user stays deactivated until manually flipped. Functionally a permanent ban without the auth.users lockdown.
- **Fix path:** parse the duration string client-side (or in plpgsql) into a concrete `timestamptz`, set `profiles.banned_until` from `mod_resolve_report`, and `auto-unban` does the rest. Custom-duration parsing should accept "24 hours" / "7 days" / "14d" / "3h" formats. Falling back to the preset enum (24h/7d/30d) is also fine if we drop custom durations. Captured here so it's not forgotten when we come back to the moderator flow.
